...
def write_report(output: Path, metadata: dict, regression: pd.DataFrame,
    ...
    methods = [
        f"Источник когорты: data/reference/client_week_panel_v2.csv ({m['panel_rows']:,} строк). Реальный период {m['start']}–{m['end']}. По каждой неделе значения fines_count и fuel_liters полностью сверены с детальными очищенными файлами: расхождений нет.",
