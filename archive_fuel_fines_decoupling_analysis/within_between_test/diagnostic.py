...
def main():
    ...
    expected_source_hash = next(
        digest for path, digest in manifest.items()
        if path.endswith("fuel_fines_decoupling_analysis/outputs/tables/client_month.csv")
    )
...
Запуск из корня проекта: `.venv/bin/python archive_fuel_fines_decoupling_analysis/within_between_test/diagnostic.py`.
'''
    (OUT/'report.md').write_text(report,encoding='utf-8')
    dump('validation.json',dict(status='PASS',upstream_client_month_hash_verified=True,source_hashes_verified=len(meta['sources']),
