...
`.venv/bin/python archive_fuel_fines_decoupling_analysis/run_analysis.py`

`.venv/bin/python archive_fuel_fines_decoupling_analysis/test_models.py -v`

FE-коэффициенты и региональные SE проверены против явной OLS с клиентскими/месячными индикаторами на тестовой панели. Conditional Poisson проверен против Poisson GLM с явными клиентскими эффектами. Исходные CSV проверяются по SHA-256, а месячные агрегаты — прямой сверкой. План анализа сохранён до моделей в analysis_plan.json. Источники и параметры — metadata.json; все CSV — tables/.
'''
