# Архив: Vehicle sensitivity — только Stage 1

Широкая гипотеза, что большие двигатели слабее сокращают объём топлива, не
подтвердилась. Узкий сигнал по вероятности покупки сохранён как диагностика.

Запуск из корня проекта:

```bash
.venv/bin/python archive_vehicle_sensitivity_analysis/diagnostic.py
```
...
сверенные с контрольным `data/reference/client_week_panel_v2.csv`; PriceShock не пересчитывается.
