...
Run from project root: .venv/bin/python archive_vehicle_sensitivity_analysis/margins_diagnostic.py
"""
import hashlib
import json
...
def verify_stage1():
    required = [
        STAGE1 / "diagnostic_results.csv",
        STAGE1 / "client_attributes.csv",
        STAGE1 / "metadata.json",
    ]
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Missing Stage 1 outputs: {missing}")
    return len(required)
...
Из корня проекта: `.venv/bin/python archive_vehicle_sensitivity_analysis/margins_diagnostic.py`.
