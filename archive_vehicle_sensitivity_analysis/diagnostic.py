"""Archived Stage 1: python archive_vehicle_sensitivity_analysis/diagnostic.py."""
...
def load_data():
    ...
    usage = json.loads((ROOT / "archive_usage_composition_analysis/outputs/metadata.json").read_text())
...
Использованы результаты очистки pipeline/clean_data.py и pipeline/preprocessing.py: fuel_clean.csv (physical_fuel_volume_main), fines_clean.csv и clients_demographics_clean.csv. Очищенные операции ранее сверены с контрольным data/reference/client_week_panel_v2.csv для каждого клиента × недели без расхождений. Месячные суммы и региональный PriceShock взяты из того же проверенного расчёта archive_fines_per_liter_analysis и проверены по SHA-256; исходные файлы не перезаписываются. PriceShock = средневзвешенная региональная цена месяца / апрельская цена региона − 1; период апрель–август 2026.

Полная исходная панель: **{meta['all_clients']:,} клиентов, {meta['all_client_months']:,} клиент-месяцев**. Аналитическая: **{meta['analytical_clients']:,} клиентов, {meta['analytical_client_months']:,} клиент-месяцев, {meta['regions']} регионов, 5 месяцев**. Исключены только {meta['excluded_missing_clients']} клиентов с пропусками engine_volume / horsepower; все месяцы оставшихся клиентов, включая нулевые покупки, сохранены. Ограничения по положительным апрельским покупкам из archive_usage_composition_analysis не переносятся. Полные суммы: {meta['full_total_liters']:,.2f} л и {meta['full_total_fines']:,} штрафов.
...
def main():
    ...
    dump('validation.json', dict(status='PASS', cleaned_source_hashes=len(meta['sources']),
        upstream_source_hashes_checked=True, independent_inference_checks=checks,
